...
%
% Further adapted by Aidan Boland
% 22 December 2019
...
\documentclass[$if(fontsize)$$fontsize$$else$11pt$endif$, a4paper]{awesome-cv-aidan}
...
\hypersetup{
    colorlinks,
    urlcolor=$if(urlcolor)$$urlcolor$$else$mainblue$endif$
}
...
\email{$email$}
$endif$
$if(emaillink)$\emaillink{$emaillink$}$endif$
$if(www)$
\homepage{$www$}
$endif$
$if(github)$
...
\makecvfooter
  {$date$}
    {$name$$if(surname)$ $surname$$endif$~~~·~~~$if(docname)$$docname$$else$CV$endif$}
  {\thepage}
